...
| `VITE_SITE_URL` | **Production SEO:** canonical URLs, Open Graph, and JSON-LD use this as your public site origin (no trailing slash). | e.g. `https://www.humanizertextai.com`. If unset, the browser’s `window.location.origin` is used (fine locally). Also update `public/sitemap.xml` and `public/robots.txt` Sitemap line to match your domain. |
